...
async function accessToken(env) {
  ...
  if (!res.ok) {
    // Google's OAuth error is a fixed enum — invalid_grant, invalid_client,
    // unauthorized_client — and naming it is the difference between a fault
    // that can be fixed and one that can only be guessed at. The enum is taken
    // and nothing else: the surrounding body can echo back a credential, and
    // error_description is free text from a third party.
    let code = null;
    try {
      const body = await res.json();
      if (body && typeof body.error === 'string' && /^[a-z_]{1,40}$/.test(body.error)) code = body.error;
    } catch { /* a non-JSON error body tells us nothing worth the risk */ }
    throw Object.assign(new Error(`token_rejected:${res.status}${code ? `:${code}` : ''}`), { status: res.status });
  }
