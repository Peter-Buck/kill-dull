...
(function(){
  ...
  function psFit(){
    ...
    var room=oy+(d.top||0)*s,i,w,last,over,wt,pad;
    for(i=0;i<psCopies.length;i++){
      w=psCopies[i];last=w.lastElementChild;
      if(!last)continue;
      w.style.transform='';w.style.paddingTop='120px';
      wt=w.getBoundingClientRect().top;
      over=last.getBoundingClientRect().bottom-wt-(room-28);
      if(over<=0)continue;
      w.style.paddingTop=Math.max(16,120-over).toFixed(0)+'px';
      over=last.getBoundingClientRect().bottom-wt-(room-28);
      if(over<=0)continue;
      // A window short enough that the block still will not fit above the
      // masses even hard against the top. Scale it about the text's own left
      // edge, so the margin stays where every other section puts it.
      pad=parseFloat(getComputedStyle(w).paddingLeft)||0;
      w.style.transformOrigin=pad+'px 0';
      w.style.transform='scale('+Math.max(0.55,
        (room-28)/(last.getBoundingClientRect().bottom-wt)).toFixed(4)+')';
    }
  }
  ...
  function psUpdate(){
    ...
    if(psCopies.length===2){
      ...
      psCopies[0].style.opacity=(1-psRamp(p,0.22,0.34)).toFixed(3);
      psCopies[1].style.opacity=psRamp(p,0.44,0.58).toFixed(3);
      psCopies[0].classList.toggle('is-on',p<0.34);
      psCopies[1].classList.toggle('is-on',p>=0.44);
    }
  ...
  function psUnlift(){
    ...
    [].slice.call(psRunway.querySelectorAll('.ps-copy')).forEach(function(w){
      var after=w.dataset.psGroup==='0'?psRunway:psRunway.nextSibling;
      while(w.firstChild)obs.insertBefore(w.firstChild,after);
    });
  ...
  function psBuild(){
    ...
    if(stage){
      ...
      // Lift the WHOLE section into the room - the chapter rule, the headline
      // and both copy blocks - so the render is the background for all of it
      // rather than a panel underneath it. The section's own vertical padding
      // goes with them, or it would leave a white band above and below.
      //
      // The premise group is inserted BEFORE the plate the four labels live in
      // and the conclusion after it, so a screen reader gets headline, premise,
      // the four Ps, conclusion, in that order. Painting order is taken back by
      // a z-index on .ps-copy, since the copy has to sit over the room.
      var obs=psRunway.closest('#observation');
      ...
      [['.kd-chapter-marker','.hero-line-wrap','.premise-lead'],['.opening-copy']]
        .forEach(function(sels,gi){
          var found=[];
          ...
          wrap.dataset.psGroup=gi;
          found.forEach(function(el){wrap.appendChild(el);});
          if(gi===0&&psPlate)stage.insertBefore(wrap,psPlate);
