...
(function(){
  ...
  function wireFourPs(){var obs=document.getElementById('observation');if(!obs)return;var objs=[],base=[],half=[],baseGap=0,ticking=false,bound=false;var W=[1,0.34,-0.34,-1],AMP=14,LAG=0.05,MINRATIO=0.55;function reduced(){return !!(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches);}function bump(t){t=t<0?0:(t>1?1:t);return 4*t*(1-t);}function measure(){var row=document.querySelector('.ps-row');objs=[];if(!row)return;var found=[].slice.call(row.querySelectorAll('.ps-object'));if(found.length!==4)return;objs=found;base=[];half=[];var i;for(i=0;i<4;i++)objs[i].style.setProperty('--ps-x','0px');for(i=0;i<4;i++){var r=objs[i].getBoundingClientRect();base.push(r.left+r.width/2);half.push(r.width/2);}baseGap=(base[1]-half[1])-(base[0]+half[0]);}function update(){ticking=false;if(objs.length!==4)return;var i;if(window.innerWidth<768||reduced()){for(i=0;i<4;i++)objs[i].style.setProperty('--ps-x','0px');return;}var room=document.querySelector('.ps-room');if(!room)return;var r=room.getBoundingClientRect(),vh=window.innerHeight||1;var p=(vh-r.top)/(vh+r.height);p=p<0?0:(p>1?1:p);var x=[];for(i=0;i<4;i++)x.push(AMP*W[i]*bump((p-i*LAG)/(1-3*LAG)));var minGap=baseGap*MINRATIO;for(var pass=0;pass<4;pass++){for(i=0;i<3;i++){var gap=(base[i+1]+x[i+1]-half[i+1])-(base[i]+x[i]+half[i]);if(gap<minGap){var push=(minGap-gap)/2;x[i]-=push;x[i+1]+=push;}}}for(i=0;i<4;i++)objs[i].style.setProperty('--ps-x',x[i].toFixed(2)+'px');}function req(){if(ticking)return;ticking=true;requestAnimationFrame(update);}function rebind(){measure();update();}rebind();if(bound)return;bound=true;window.addEventListener('scroll',req,{passive:true});window.addEventListener('resize',function(){measure();req();});window.addEventListener('load',function(){setTimeout(rebind,0);});try{new MutationObserver(function(){setTimeout(rebind,0);}).observe(obs,{childList:true});}catch(e){}setTimeout(rebind,300);setTimeout(rebind,1200);setTimeout(rebind,2200);}
