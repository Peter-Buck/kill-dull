...
(function(){
  ...
  function psFit(){
    ...
    var oy=Math.max(Math.min(vh*0.70-d.anchor*s,0),vh-d.h*s);
    psPlate.style.setProperty('--ps-oy',oy.toFixed(1)+'px');
    // The copy sits in the clear room ABOVE the masses, so it has to be sized
    // against where they actually are. A percentage of the viewport parts
    // company with them on a tall screen, where the clamp above lifts the
    // plate: at 1280x1024 it left the headline 7px clear of PRODUCT.
    psRunway.style.setProperty('--ps-copy-h',
      Math.max(140, oy + (d.top || 0)*s - 26).toFixed(0) + 'px');
  }
