...
(function(){
  ...
  style.textContent='@keyframes kd-terminal-compound{0%{transform:scale(1)}50%{transform:scale(1.175)}100%{transform:scale(1)}}'+
    ...
    '#observation.ps-scroll-ready .pitch-scroll-runway{position:relative;height:var(--pitch-runway-h,1100px)}'+
    '#observation.ps-scroll-ready .pitch-scroll-stage{position:sticky;top:24px;z-index:2;background:var(--paper)}'+
    '#observation.ps-scroll-ready .home-ps{height:var(--pitch-grid-h,320px)!important;align-items:start!important;overflow:hidden!important}'+
    '#observation.ps-scroll-ready .home-ps>div{will-change:transform;transform:translate3d(0,var(--pitch-y,0px),0)}'+
  ...
  function wireScrollRise(root,opts){if(!root||root.dataset.scrollWired==='1')return;root.dataset.scrollWired='1';root.classList.add(opts.readyClass);var grid=root.querySelector(opts.stepsSel),stop=root.querySelector(opts.stopSel);if(!grid||!stop)return;var runway=document.createElement('div');runway.className='pitch-scroll-runway';var stage=document.createElement('div');stage.className='pitch-scroll-stage';var first=opts.startSel?root.querySelector(opts.startSel):root.firstElementChild;if(!first)return;var nodes=[],node=first;while(node&&node!==stop){nodes.push(node);node=node.nextElementSibling;}root.insertBefore(runway,first);runway.appendChild(stage);for(var n=0;n<nodes.length;n++)stage.appendChild(nodes[n]);var steps=stage.querySelectorAll(opts.stepsSel+'>div');if(!steps.length)return;var stagger=90,startOffset=220,settle=startOffset+(steps.length-1)*stagger+50;if(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;var ticking=false,travel=560;function clamp(v,min,max){return Math.max(min,Math.min(max,v));}function update(){ticking=false;if(window.innerWidth<768)return;var rect=runway.getBoundingClientRect(),stickyTop=24,scrolled=clamp(stickyTop-rect.top,0,travel),progress=scrolled/travel,maxY=0;for(var i=0;i<steps.length;i++){var y=Math.max(0,startOffset+(i*stagger)-(progress*settle));maxY=Math.max(maxY,y);steps[i].style.setProperty('--pitch-y',y.toFixed(1)+'px');}var base=opts.gridBase;if(base==='auto'){base=0;for(var j=0;j<steps.length;j++)base=Math.max(base,steps[j].offsetHeight);}var gridHeight=base+Math.min(370,maxY);grid.style.setProperty('--pitch-grid-h',gridHeight.toFixed(1)+'px');var stageHeight=stage.offsetHeight;runway.style.setProperty('--pitch-runway-h',(stageHeight+travel).toFixed(1)+'px');}function requestUpdate(){if(ticking)return;ticking=true;requestAnimationFrame(update);}update();window.addEventListener('scroll',requestUpdate,{passive:true});window.addEventListener('resize',requestUpdate);return function(){window.removeEventListener('scroll',requestUpdate);window.removeEventListener('resize',requestUpdate);};}
  function wirePitchScroll(){wireScrollRise(document.getElementById('pitch-intro'),{readyClass:'pitch-scroll-ready',stepsSel:'.pitch-steps',stopSel:'.pitch-actions',gridBase:150});}
  function wirePremiseScroll(){var obs=document.getElementById('observation');if(!obs)return;var teardown=null;function attempt(){var o=document.getElementById('observation');if(!o)return;if(o.querySelector('.pitch-scroll-runway'))return;if(!o.querySelector('.home-ps')||!o.querySelector('.opening-copy')||!o.querySelector('.hero-line-wrap'))return;if(teardown){teardown();teardown=null;}o.dataset.scrollWired='';teardown=wireScrollRise(o,{readyClass:'ps-scroll-ready',stepsSel:'.home-ps',stopSel:'.opening-copy',startSel:'.hero-line-wrap',gridBase:'auto'})||null;}attempt();window.addEventListener('load',function(){setTimeout(attempt,0);});try{new MutationObserver(function(){setTimeout(attempt,0);}).observe(obs,{childList:true});}catch(e){}setTimeout(attempt,300);setTimeout(attempt,1200);setTimeout(attempt,2200);}

  normalizeShell();normalizeCtas();wireRouteTransition();wireReadingFit();var core=document.createElement('script');core.src='/assets/kd-core.js';core.onload=function(){normalizeShell();normalizeCtas();document.head.appendChild(style);wireRouteTransition();wirePitchScroll();wirePremiseScroll();normalizeCtas();};document.head.appendChild(core);
})();